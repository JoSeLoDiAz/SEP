...
// ── Admin home ────────────────────────────────────────────────────────────────
// Datos mock de la última convocatoria — se reemplazan en Fase 2 con datos reales.
const ADMIN_KPIS_MOCK = {
...
const ADMIN_KPI_CARDS: Array<{
  ...
  { label: 'Proyectos totales',  value: ADMIN_KPIS_MOCK.total,        icon: FolderKanban, bg: 'bg-[#00304D]/10', fg: 'text-[#00304D]' },
  { label: 'Confirmados',        value: ADMIN_KPIS_MOCK.confirmados,  icon: CheckCircle2, bg: 'bg-blue-50',      fg: 'text-blue-600'  },
  { label: 'Aprobados',          value: ADMIN_KPIS_MOCK.aprobados,    icon: TrendingUp,   bg: 'bg-emerald-50',   fg: 'text-emerald-600' },
  { label: 'Rechazados',         value: ADMIN_KPIS_MOCK.rechazados,   icon: XCircle,      bg: 'bg-red-50',       fg: 'text-red-600'   },
]
...
const ADMIN_CARDS: AdminCard[] = [
  ...
  },
  {
    ...
    color: '#0070C0',
    links: [
      { label: 'Asignar Logos Capacitadores',     disabled: true },
      { label: 'Asignar proyectos a Usuarios',    disabled: true },
      { label: 'Asignar Convenios',               disabled: true },
      { label: 'Gestionar Convocatorias',         disabled: true },
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Gestión de Convenios',     disabled: true },
...
function AdminHome() {
  return (
    <div className="p-5 sm:p-7 xl:p-10 flex flex-col gap-6">
      ...
      {/* KPIs — última convocatoria (datos en maqueta) */}
      <section className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-neutral-400">Última convocatoria</span>
            <span className="text-base font-bold text-[#00304D] font-mono">{ADMIN_KPIS_MOCK.convocatoria}</span>
          </div>
          <span className="inline-flex items-center gap-1.5 text-[10px] text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-full font-semibold">
            DATOS DE PRUEBA
          </span>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {ADMIN_KPI_CARDS.map((kpi) => (
            <div key={kpi.label}
